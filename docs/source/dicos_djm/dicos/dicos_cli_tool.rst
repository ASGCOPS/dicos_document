...
1. [Apply for certificate](wiki:/apply_for_certificate)
2. [Import certificate to browser](wiki:/import_certificate_to_browser)
3. [Join Virtual Organization](wiki:/join_virtial_organization)
...
You should already have a membership of specific VO (Virtual Organization, usually `twgrid`) associated with your user certificate. (If not, see [here](wiki:/account))
...
__**Troubleshooting.**__ When you see this:
...
###status
Check status of job.
The default subcommand just shows jobs within 240 hours, and 15 jobs at most.
